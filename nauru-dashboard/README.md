...
- **Existing / Committed**, 114 named projects, 2017 to 2026, multi-donor.
...
It has seven tabs plus a small rule-based chatbot that answers questions from
whatever data is currently loaded. The first two tabs each analyse one track
only and never compare or combine the two: **Master List Overview**
(existing/committed rows) and **RONAdapt II Priorities** (pipeline rows). The
rest combine both tracks, as before: Money Flow and Gaps, Project Master List
(the full sortable/filterable table), Sectors, Duplicates, and Hazards &
Funding Outcomes.
...
├── data/
│   ├── projects.csv         the bundled data snapshot (511 rows)
...
- `Confirmed overlap - excluded from adjusted totals`, this row is
  excluded from the "adjusted combined total" figure shown on the
  Duplicates tab (its capital still counts everywhere else, sector totals,
  the Project Master List, the Master List Overview and RONAdapt II
  Priorities tabs, etc., it's only removed from that one headline
  "adjusted" figure, to avoid double-counting the same money twice when
  adding the two tracks together).
...
the same way and the dashboard (the Duplicates tab and the chatbot's "is
anything double-counted?" answer) will pick it up
automatically.
...
3. Click through all seven tabs and confirm the URL hash updates
   (`#overview`, `#ronadapt`, `#flows`, `#projects`, `#sectors`,
   `#duplicates`, `#hazards`) and the right content shows.
